...
type ComputedAggregatedEpochData<Txs> = {
  transactionsWithClaimable: Txs[];

  refundedByChain: { [chainId: number]: string };
  claimableByChain: { [chainId: number]: string };
};
type ComputeAggregatedStakeChainDetailsResult<Txs> = {
  [epoch: number]: ComputedAggregatedEpochData<Txs>;
};
...
export function computeAggregatedStakeChainDetails(
...
): ComputeAggregatedStakeChainDetailsResult<TransactionWithCaimableByStakeChain> {
  ...
  const entries: [
    number,
    ComputedAggregatedEpochData<TransactionWithCaimableByStakeChain>,
  ][] = Object.keys(transactionsWithClaimableByEpoch).map(_epoch => {
...
export function computeAggregatedStakeChainDetails_V3(
  ...
  options?: ComputationOptions,
): ComputeAggregatedStakeChainDetailsResult<TransactionWithCaimableByStakeChain_V3> {
  const roundBignumber = options?.roundBignumber ?? defaultRounder;
  ...
  const entries: [
    number,
    ComputedAggregatedEpochData<TransactionWithCaimableByStakeChain_V3>,
  ][] = Object.keys(transactionsWithClaimableByEpoch).map(_epoch => {
    const epoch = Number(_epoch);
    return [
      epoch,
